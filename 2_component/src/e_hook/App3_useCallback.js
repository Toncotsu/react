...
***************** hook 
    ...
    useCallback()
    - 주로 랜더링 상황을 최적화해야하는 상황에 사용 -> 잘못하면 오히려 성능 저하 될 수 있음 
    - useMemo() 유사
...
const Average = ()=>{
    ...
    const onChange = useCallback((evt)=>{ setNum(evt.target.value)},[])
    const onInsert = useCallback(()=>{list.push(parseInt(num))
        console.log(list)
        //setList(list);
        setList([...list]);
        setNum('')},[num,list])
